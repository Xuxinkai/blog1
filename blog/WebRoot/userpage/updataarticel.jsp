...
<head>
    ...
    <title>徐新凯个人博客后台管理-文章查看与修改</title>
...
<div class="admin">
	<div class="line-big">
        <div class="xm9" style="width: 75%;margin-left: auto;margin-right: auto;float: none;">
			<form name="example" id="form" method="post" action="UpdataArticel" >
			<input type="hidden" id="aid" name="aid" value="${ab.aid }" />
			<table class="table" style="width: 100%;" >
				<tr>
					<td>文章题目：</td>
					<td><input type="text" id="articeltitle" name="articeltitle" class="input" value="${ab.title }" /></td>
					<td>所属栏目：</td>
					<td><select name="tid" id="tid">
					<c:forEach items="${articeltypes }" var="st">
					
					<option value="${st.tid }" <c:if test="${st.tid == ab.tid }">selected="selected"</c:if>>${ st.tname}</option>
					</c:forEach>
					</select>
				...
				<tr>
					<td>作者：</td>
					<td><input type="text" id="zuozhe" name="zuozhe" class="input" value="${ab.author }" /></td>
					<td>时间：</td>
					<td><input class="datainp" id="datebut" name="date" type="text" value="${ab.time }" readonly>
				...
				<tr>
				<td colspan="4">
				<textarea id="content1" name="content1" cols="100" rows="8" style="width:100%;height:600px;">${ab.content }</textarea>
				<br />
